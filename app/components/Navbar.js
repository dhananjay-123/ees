    ...
    <>
      <nav className="fixed top-0 w-full h-16 bg-[#F1F5F9]/80 backdrop-blur-md border-b border-[#E2E8F0] z-[2000] px-6 lg:px-12 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-[#059669] rounded-lg flex items-center justify-center text-white shadow-lg shadow-[#059669]/20">
          ...
          {links.map((link) => (
            <Link
              ...
              className={`text-[10px] font-bold uppercase tracking-[0.2em] transition-all relative py-1 ${
                pathname === link.href ? "text-[#059669]" : "text-[#64748B] hover:text-[#0F172A]"
              }`}
            >
              {link.name}
              {pathname === link.href && (
